...
print('[*DEEPCAT*] Your using DeepCat-V0.83BS-G2V')
print('[*DEEPCAT*] Supported resolution: 256x256. For 512x512, use the XDeepCat, please.')
...
save_path = os.path.join(output_folder, "generated_image_256x256_deepcat.png")
generate_and_save_image_from_text(text_description, save_path)
upscl.upscale('deepcat')
wmarker.watermark_with_transparency('./results/sr_image.png','./results/sr_image.png','wmark.png',(0,0))
print('[*DEEPCAT*] Model training successful! Open DeepCat-Console?')
print('[*DEEPCAT-TIPS*] Always images with extra-size (1024x1024) saving in /results and normal images (256x256) in /generated_images')
answer=input('No/Yes(N/Y)>> ')
if answer.lower()=='n':
    print('[*DEEPCAT*] Ok. Closing...')
    exit()
elif answer.lower()=='y':
    print('[*DEEPCAT*] Opening DeepCat-Console ...')
    while True:
        input_of_user = input('[-=User=-] >> ')
        generate_and_save_image_from_text(text_description, save_path)
        upscl.upscale('deepcat')
        wmarker.watermark_with_transparency('./results/sr_image.png','./results/sr_image.png','wmark.png',(0,0))
        print('[-=DeepCat Console=-] Generation success!')
else:
    exit()
